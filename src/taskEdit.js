...
export class taskEdit {
  ...
  taskEditForm(
    taskDetailsToEdit //This will bring up the task input form with the existing task details
  ) {
    const popover = document.querySelector(".newTaskDiv");
    const popupOpened = popover.togglePopover();

    let form = document.querySelector("#newTaskForm");
    form.querySelector("select[name='projectName']").value =
      taskDetailsToEdit["projectName"];
    form.querySelector("input[name='taskTitle']").value =
      taskDetailsToEdit.taskTitle;
    form.querySelector("input[name='taskDetails']").value =
      taskDetailsToEdit.taskDetails;
    form.querySelector("input[name='dueDate']").value =
      taskDetailsToEdit.dueDate;
    form.querySelector("select[name='priority']").value =
      taskDetailsToEdit.priority;
  ...
  saveAmendedTask(taskIndex) {
    ...
    document.querySelector(".editTask").addEventListener("click", () => {
            ...
            tasks: [
              {
                ...
                projectName: addNewTaskFormData
                  .getAll("projectName")
                  .toString(),
              },
            ],
      ...
      currentStorage["completeTaskList"][taskIndex[0]]["tasks"][taskIndex[1]] =
        newTaskObject["completeTaskList"][0]["tasks"][0];
